/* ============================================================================
   THE NINETEEN MARKS — Furniture. Same names as Agate's ICON_PATHS so a call
   site swaps sets without renaming, but STROKED (1.6, round caps and joins)
   rather than cut. Agate's set was right angles and 45° edges only "because
   the system has no radius, so a rounded corner would be the only curve on
   screen"; Furniture has a 16px radius everywhere, so cut marks fight it.

   There is no icon library. The app's drift guard bans Lucide by name
   precisely so this set stays first-party — adding a mark means drawing it on
   the 16px grid here, not installing a dependency.
   ========================================================================= */
...
export const FX_ICON_NAMES = Object.keys(FX_ICON_PATHS);
